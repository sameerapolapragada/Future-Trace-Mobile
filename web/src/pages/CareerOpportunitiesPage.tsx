...
function OpportunityCard({ role }: { role: CareerOpportunityRole }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-navy-card/90 p-4">
      ...
      <Link
        to={rolePath}
        className="mt-3 flex items-start justify-between gap-2 ft-focus-ring"
      >
      ...
      {loading || !opportunities ? (
        ...
        <div className="space-y-4 pb-4">
          ...
          <div className="space-y-3">
            {opportunities.recommendedRoles.map((role) => (
              <OpportunityCard key={role.title} role={role} />
            ))}
          </div>

          <p className="flex items-start gap-2 text-center text-[9px] leading-relaxed text-muted">
            ...
            <span className="text-left">
              Match scores are based on your profile and real-time market data. Roles and scores may
